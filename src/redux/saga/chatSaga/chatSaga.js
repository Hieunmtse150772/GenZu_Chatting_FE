...
function* setEmoji(action) {
  try {
    ...
    switch (type) {
      case 'ADD':
        apiCall = call(addEmoji, rest.id_message, rest.emoji)
        yield call([socket, 'emit'], 'add emoji', apiCall)
        break
      case 'UPDATE':
        apiCall = call(updateEmoji, rest.id_emoji, rest.emoji)
        yield call([socket, 'emit'], 'edit emoji', apiCall)
        break
      default: // Default là "DELETE"
        apiCall = call(deleteEmoji, rest.id_message, rest.id_emoji)
        yield call([socket, 'emit'], 'delete emoji', apiCall)
    }
