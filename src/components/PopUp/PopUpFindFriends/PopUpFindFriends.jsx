    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
      ...
      >
        ...
        <div className='mx-auto mt-10 max-w-md rounded-lg bg-white p-6 shadow-xl'>
          ...
          {message.text && (
            <div
              className={`slideIn relative z-50 mt-2 flex justify-between rounded border px-4 py-3 ${message.isSuccess ? 'border-green-400 bg-green-200 text-green-700' : 'border-red-400 bg-red-200 text-red-700'}`}
              role='alert'
            >
              <span className='block sm:inline'>{message.text}</span>
              <span className='absolute bottom-0 right-0 top-0 ml-4 px-1 py-3'>
                <svg
